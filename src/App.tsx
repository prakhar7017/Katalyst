...
import ComposioCallback from "./components/ComposioCallback";
...
function Dashboard() {
  ...
  const { upcomingEvents, pastEvents, isLoading, error, refreshEvents } =
    useCalendarMCP(5);
  const [selectedMeeting, setSelectedMeeting] = useState<CalendarEvent | null>(null);
  const [isViewingPastMeeting, setIsViewingPastMeeting] = useState(false);

  const handleMeetingClick = (meeting: CalendarEvent, isPast: boolean = false) => {
    setSelectedMeeting(meeting);
    setIsViewingPastMeeting(isPast);
  };
        ...
        <div className="px-4 py-6 sm:px-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <MeetingList
              ...
              isLoading={isLoading}
              error={error}
              onMeetingClick={(meeting) => handleMeetingClick(meeting, false)}
            />
  ...
  return (
    <Router>
      <Routes>
        ...
        <Route path="/auth/callback" element={<ComposioCallback />} />
      </Routes>
    </Router>
  );
}
